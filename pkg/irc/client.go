...
func (c *Client) listenToChannels(cancel context.CancelFunc) {
	for {
		...
		case line := <-c.server.recvChan:
			...
			case
				RPL_ERRORJOIN, ERR_BADMASK, ERR_CANNOTSENDTOCHAN, ERR_CODE, ERR_NORECIPIENT,
				ERR_NOSUCHCHANNEL, ERR_NOSUCHNICK, ERR_NOSUCHSERVER, ERR_NOTEXTTOSEND, ERR_NOTOPLEVEL,
				ERR_TOOMANYCHANNELS, ERR_WASNOSUCHNICK:
				if callback, ok := c.callbackHandlers[EventError]; ok {
					callback(EventType{
						Server:  line.ServerName,
						Room:    line.Room,
